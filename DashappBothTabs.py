...
app.layout = html.Div([
    ...
    dbc.Row([
        ...
        dcc.Tabs(id="tabs", value='tab-2', children=[
            dcc.Tab(label='SHAP', value='tab-1'),
            dcc.Tab(label='LIME', value='tab-2'),
            ]),
...
@app.callback(Output('fix this', 'figure'),
 #   [Output('Metric_1', 'figure'),
  #   Output('main_text', 'children'),
   #  Output('Graph', 'figure')],
    [Input('data_patient', 'children')])
...
def render_content(tab,data_patient):
    if tab == 'tab-1':
        prediction = predict_lime_summary(data_patient)
        return html.Div([
            html.H3('LIME Explanation'),
            prediction
        ])
    elif tab == 'tab-2':
        prediction1 = predict_shap_summary(data_patient)
        return html.Div([
